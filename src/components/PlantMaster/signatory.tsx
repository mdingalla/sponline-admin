        ...
        if(id){
            PlantMaster.GetPlant(id)
                .then((plant)=>{
                    Promise.all(
                        [   
                            IsPeople(plant.RegionalFinanceId) ? UserAccessApi.PeopleToPicker(plant.RegionalFinanceId) : Promise.resolve(null),
                            IsPeople(plant.FinanceControllerId) ? UserAccessApi.PeopleToPicker(plant.FinanceControllerId) : Promise.resolve(null),
                            IsPeople(plant.HRITMGRPAYROLLId) ? UserAccessApi.PeopleToPicker(plant.HRITMGRPAYROLLId) : Promise.resolve(null),
                            IsPeople(plant.HRITMGRId) ? UserAccessApi.PeopleToPicker(plant.HRITMGRId) : Promise.resolve(null)
                        ]
                    ).then((results)=>{

                        const regionalfinance = results[0];
                        const financecontroller  = results[1];
                        const hrpayroll = results[2];
                        const hritmgr = results[3];
